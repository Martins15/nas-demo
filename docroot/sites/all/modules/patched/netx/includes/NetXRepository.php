...
class NetXRepository {
   ...
   *
   * Up until >=8.3.0, cookie should be used instead of sessionKey for authentication.
   *
   * @var string
   */
  protected $cookie;
  ...
  /**
   * SessionKey for authentication call.
   *
   * Started from 8.3.0+, sessionKey should be used instead of cookie for authentication.
   *
   * @var string
   */
  ...
  /**
   * Authentication type. Can be "cookie" or "call".
   *
   * @var string
   */
  protected $authType;
   ...
   */
  public function __construct($server = NULL, $auth_type = NULL) {
    if (empty($server)) {
    ...
    if (empty($auth_type)) {
      $this->authType = netx_get_authentication_type();
    }
    else {
  ...
  protected function ensureAuthorized() {
    ...
    if (!empty($this->cookie)) {
      // Parse cookies and identify whether they are expired or not.
      $cookies_raw = explode(';', $this->cookie);
      foreach ($cookies_raw as $cookie_item) {
        $parts = explode('=', $cookie_item);
        if (count($parts) == 2 && trim($parts[0]) == 'Expires') {
    ...
    // Add additional argument for new versions.
    if ($this->authType == NETX_AUTHENTICATION_TYPE_NEW) {
      $session_key = $this->authenticate($this->user, $this->pass);
      if ($session_key) {
  ...
  protected function doRequest($url, $options, $decode_data = TRUE) {
    ...
    if (variable_get('netx_log_requests', FALSE)) {
      // Prepare data to log.
      $response_dump = (array) $response;
      // TODO: make the size configurable?
      // Limiting the size to 50Kb.
      $response_limit = 50 * 1024;
      if (isset($response_dump['data']) && strlen($response_dump['data']) > $response_limit) {
        $response_dump['data'] = substr($response_dump['data'], 0, $response_limit);
      }

      watchdog('netx', 'Response takes %time for %url response %response', array(
        '%time' => sprintf('%.3fs', $time['time'] / 1000),
        '%url' => $url,
        '%response' => var_export($response_dump, TRUE),
      ), WATCHDOG_DEBUG);
    }
  ...
  protected function doRequestJsonRPC($method, $params, $send_cookie = TRUE) {
    // Authenticate user using sessionKey.
    if ($this->authType == NETX_AUTHENTICATION_TYPE_NEW
    ...
    $data = drupal_json_encode($data_raw);
    $options = array(
      'method' => 'POST',
      'data' => $data,
  ...
  public function authenticate($user, $pass) {
    $response = $this->doRequestJsonRPC('authenticate', array($user, $pass), FALSE);
    if ($response->data['result']) {
      // Update credentials to reuse repo.
      $this->user = $user;
      $this->pass = $pass;
  ...
  /**
   * Validate authentication for given authentication type and credentials.
   *
   * @param string $auth_type
   *   Authentication type: "call" or "cookie".
   * @param string $user
   *   Username.
   * @param string $pass
   *   Password.
   *
   * @return bool
   *   Whether user has been validated or not.
   */
  public function authenticationValidation($auth_type, $user, $pass) {
    // Check cookie validation.
    $cookies_auth = FALSE;
    $cookies_auth_response = $this->authenticateAndGetUserBean($user, $pass);
    if (!empty($cookies_auth_response)) {
      $cookies_auth = TRUE;
    }

    // Check sessionKey validation.
    $session_key_auth = TRUE;
    if ($auth_type == NETX_AUTHENTICATION_TYPE_NEW) {
      $session_key_response = $this->authenticate($user, $pass);
      if (empty($session_key_response) || $session_key_response == "-1") {
        $session_key_auth = FALSE;
      }
    }

    return $cookies_auth & $session_key_auth;
  }

  /**
   * Set cookies for current repository.
   */
